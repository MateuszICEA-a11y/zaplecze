...
/* Edytor wpisu Content Watchera – logika 1:1 ze <script> w
   dashboard/app/src/pages/[domain]/content-watcher/edytor.astro, opakowana
   w mount(). Uruchamia ją LegacyHost po wstawieniu znaczników (edytor.html).
   Do przepisania na React po kawałku – na razie celowo bez zmian w logice. */
export async function mount(): Promise<void> {
  ...
  function docSnapshot() {
    ...
    for (const node of $('[data-ed-doc]')!.querySelectorAll<HTMLElement>('.ed-doc-section, .ed-faq-head')) {
  ...
  /* Wymiany spółgłoskowe na końcu rdzenia: „fotowoltaika" → „w fotowoltaice",
     ...
     pokrycie i model dostawał sprzeczny sygnał: napisał poprawnie po polsku,
     a panel pokazywał brak frazy. Lustro `soft()` w pipeline/matching.py. */
  const SOFT_ENDINGS: [string, string][] = [['dz', 'g'], ['sz', 'ch'], ['c', 'k']];

  function soft(word: string) {
  ...
  /* Surowa fraza z wyszukiwarki („leady fotowoltaika") wchodzi do tekstu
     odmieniona i z przyimkiem („leadów na fotowoltaikę") – tak każe reguła
     redakcyjna. Matcher musi to znosić: przyimki i spójniki mogą stać między
     słowami frazy (maks. dwa), a w samej frazie są ignorowane. */
  const PHRASE_FILLERS = new Set(['na', 'w', 'we', 'z', 'ze', 'do', 'dla', 'o', 'u', 'od', 'po',
  ...
  function findPhrase(text: string, phrase: string) {
    const needle = tokens(phrase).map((token) => token.stem).filter((word) => !PHRASE_FILLERS.has(word));
    if (!needle.length) return [];
    const hay = tokens(text);
    const hits: { start: number; end: number }[] = [];
    for (let i = 0; i < hay.length; i += 1) {
      if (hay[i].stem !== needle[0]) continue;
      let last = i;
      let ok = true;
      for (let n = 1; n < needle.length && ok; n += 1) {
        let j = last + 1;
        let gap = 0;
        ...
        if (j < hay.length && hay[j].stem === needle[n]) last = j;
        else ok = false;
      }
      if (ok) hits.push({ start: hay[i].start, end: hay[last].end });
    }
    return hits;
  }
  ...
  function phraseVariant(text: string, phrase: string) {
  ...
  /** Pierwszy zakres tekstu z tą frazą w dokumencie – od miejsca, w którym
      stoi widok, żeby kolejne kliknięcia szły po następnych wystąpieniach. */
  ...
  function renderScore() {
    ...
    list.replaceChildren(...targets.map((row) => {
      ...
      // jest w treści, ale wpis nie jest na nią widoczny. Brak to czerwony
      // minus, nie plus: plus czytało się jak „dodaj", a to jest stan braku.
      ...
      const variant = used ? phraseVariant(text, row.keyword) : null;
      if (variant && normalize(variant) !== normalize(row.keyword)) {
        const usedAs = document.createElement('small');
        usedAs.className = 'ed-kw-variant';
        usedAs.textContent = `w tekście: „${variant}"`;
        name.append(usedAs);
      }
      ...
      if (used) {
        ...
        item.title = 'przejdź do tej frazy w treści';
        item.addEventListener('click', () => focusKeyword(row.keyword));
  ...
  async function wpApply(force = false) {
    ...
    try {
      ...
      if (authorBody.author_id && content) {
        ...
        if (who) $('[data-ed-author]')!.textContent = who.name;
      }
  ...
  function render(current: Job) {
    ...
    // Po zakończonym przebiegu analizy zrobiły swoje – zwijamy je, żeby droga
    // do dokumentu była krótka. Ręczne rozwinięcie ma pierwszeństwo.
    if (current.status === 'done') {
      autoFold($('[data-ed-serp]'), true);
      autoFold($('[data-ed-rivals]'), true);
    }
  }

  /* ---------- zwijanie sekcji analitycznych ---------- */

  function foldButton(section: HTMLElement) {
    return section.querySelector<HTMLButtonElement>('.ed-serp-fold');
  }

  function autoFold(section: HTMLElement | null, fold: boolean) {
    if (!section || section.dataset.foldTouched) return;
    section.classList.toggle('is-folded', fold);
    const button = foldButton(section);
    if (button) button.textContent = fold ? 'rozwiń' : 'zwiń';
  }

  for (const section of document.querySelectorAll<HTMLElement>('[data-ed-serp], [data-ed-rivals]')) {
    foldButton(section)?.addEventListener('click', () => {
      section.dataset.foldTouched = '1';
      const folded = section.classList.toggle('is-folded');
      const button = foldButton(section);
      if (button) button.textContent = folded ? 'rozwiń' : 'zwiń';
    });
  }
  ...
  /* ---------- podgląd całości + eksport do Google Docs ----------
     Wdrożenie i korekta odbywają się poza edytorem, więc docelowa treść musi
     dać się przeczytać jednym ciągiem i zabrać ze sobą: schowek niesie HTML
     (Google Docs przy wklejeniu zachowuje nagłówki, listy i linki), a .doc
     otwiera się i w Wordzie, i przez „Otwórz w Dokumentach Google". */

  /** Kopia dokumentu bez atrybutów interfejsu – czyste tagi i linki. */
  function previewSnapshot() {
    const box = docSnapshot();
  ...
  function openPreview() {
    const host = $('[data-ed-preview-body]')!;
    host.replaceChildren(...previewSnapshot().childNodes);
    const words = (host.textContent?.match(/[\p{L}\p{N}][\p{L}\p{N}'’-]*/gu) ?? []).length;
    const rejected = job?.sections?.filter((section) => section.decision === 'rejected').length ?? 0;
    $('[data-ed-preview-note]')!.textContent = `${pl.format(words)} słów · wersja po Twoich decyzjach`
      + (rejected ? ` · ${pl.format(rejected)} odrzuconych sekcji w dotychczasowym brzmieniu` : '');
    $('[data-ed-preview]')!.hidden = false;
    document.body.classList.add('ed-preview-lock');
  }

  function closePreview() {
    $('[data-ed-preview]')!.hidden = true;
    document.body.classList.remove('ed-preview-lock');
  }

  $('[data-ed-preview-open]')?.addEventListener('click', openPreview);
  $('[data-ed-preview-close]')?.addEventListener('click', closePreview);
  $('[data-ed-preview]')?.addEventListener('click', (event) => {
    if (event.target === event.currentTarget) closePreview();
  });
  document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && !$('[data-ed-preview]')!.hidden) closePreview();
  });

  $('[data-ed-preview-copy]')?.addEventListener('click', async (event) => {
    const button = event.currentTarget as HTMLButtonElement;
    const body = $('[data-ed-preview-body]')!;
    const html = `<meta charset="utf-8">${body.innerHTML}`;
    try {
      await navigator.clipboard.write([new ClipboardItem({
        'text/html': new Blob([html], { type: 'text/html' }),
        'text/plain': new Blob([body.textContent ?? ''], { type: 'text/plain' }),
      })]);
      button.textContent = 'skopiowano – wklej w docs.new';
    } catch {
      // Starsza przeglądarka bez ClipboardItem – zostaje czysty tekst.
      await navigator.clipboard.writeText(body.textContent ?? '');
      button.textContent = 'skopiowano jako tekst';
    }
    setTimeout(() => (button.textContent = 'kopiuj do Google Docs'), 2500);
  });

  $('[data-ed-preview-doc]')?.addEventListener('click', () => {
    const body = $('[data-ed-preview-body]')!;
    const title = (entry?.title ?? 'artykuł').replace(/[<>&]/g, '');
    const slug = title.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/ł/g, 'l')
      .replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'artykul';
    const html = `<html><head><meta charset="utf-8"><title>${title}</title></head><body>${body.innerHTML}</body></html>`;
