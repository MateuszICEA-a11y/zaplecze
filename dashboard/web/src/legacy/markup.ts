/* Znaczniki przeniesionego edytora wpisu (build time) – z podstawioną domeną.
   Dwie części, bo między nimi stoją już panele React (edytor/PostEditor.tsx):
   belka pipeline'u i karty etapów końcowych w kolumnie bocznej. */
import "server-only";
import { readFileSync } from "node:fs";
import { resolve } from "node:path";

export type EditorMarkup = { top: string; side: string };
...
export function editorMarkup(domain: string): EditorMarkup {
  return { top: read("edytor-top", domain), side: read("edytor-side", domain) };
}
