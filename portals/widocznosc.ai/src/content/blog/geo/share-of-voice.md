...
W ICEA każdy audyt SoV przebiega według tego samego schematu. Każdy krok ma konkretny rezultat.
...
2. **Zaprojektowanie liczebności próby** – każde pytanie uruchamiamy minimum 5 razy w każdej z 4 platform (ChatGPT, Claude, Gemini, Perplexity). Dla 30 pytań × 5 uruchomień × 4 platformy = 600 testów. To minimum, żeby wyniki probabilistyczne były stabilne.
3. **Pobieranie (scraping) i normalizacja danych** – każdą odpowiedź zapisujemy strukturalnie: tekst, lista cytowanych URL-i, lista marek, wydźwięk. Normalizacja jest kluczowa – „Tesla" w odpowiedzi może odnosić się do firmy lub do Modelu 3, więc używamy NLP do rozróżnienia kontekstu.
...
5. **Monitorowanie w czasie** – pojedynczy pomiar to punkt odniesienia. Realna wartość pojawia się przy trzecim lub czwartym pomiarze, kiedy widać trend. Miesięczny ponowny pomiar tej samej puli to standard.
...
Trzy pomiary z trendem 12% → 15% → 18% przez kwartał to twardy dowód, że strategia GEO działa. Trzy pomiary 22% → 19% → 16% w tym samym okresie to alarm – konkurencja wdraża coś, czego my nie obsługujemy.
...
W pełnym audycie widoczności w AI w ICEA każdy raport zawiera te trzy metryki w trzech ujęciach: Twoja marka, top 5 konkurencji, dynamika kwartalna. Plus szczegółowy podział na platformy – bo SoV w ChatGPT i SoV w Perplexity to różne historie, czasem drastycznie się różniące. Jeśli chcesz zobaczyć, jak wygląda Twoja marka w tym układzie, zacznij od darmowego [Widoczność marki w AI](/narzedzia/brand-check/) – w 60 sekund dostaniesz pierwszy obraz SoV w 4 silnikach AI.
