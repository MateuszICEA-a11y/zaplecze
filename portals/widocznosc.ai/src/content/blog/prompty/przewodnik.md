...
Prompt engineering (inżynieria podpowiedzi) to dyscyplina, która decyduje o tym, czy LLM (Large Language Model, czyli duży model językowy) da Ci użyteczną odpowiedź, czy bezużyteczny szum. Nie chodzi o magiczne słowa – chodzi o zrozumienie, jak model przetwarza instrukcje, i o zaprojektowanie promptu tak, żeby model dostał dokładnie to, czego potrzebuje. Ten przewodnik prowadzi Cię od podstawowych technik przez zaawansowane architektury wnioskowania, aż po bezpieczeństwo systemów opartych na LLM – z konkretnymi przykładami gotowymi do wdrożenia.

## Czym jest inżynieria podpowiedzi i dlaczego ma znaczenie

Prompt to każda instrukcja, pytanie lub kontekst, który przekazujesz modelowi AI. Brzmi banalnie. Ale jakość promptu bezpośrednio determinuje jakość odpowiedzi – i to w sposób, który da się zmierzyć i zoptymalizować.
...
Nowoczesny model widzi trzy rodzaje wiadomości: systemową (`system`), użytkownika (`user`) i asystenta (`assistant`). Wiadomość systemowa ustawia rolę i reguły zachowania – jest najsilniejsza hierarchicznie. Wiadomość użytkownika to Twoje zapytanie. Odpowiedź asystenta to to, co model wygenerował.
...
- **Format wyjścia** – jak ma wyglądać odpowiedź; lista, tabela, JSON, akapit o dokładnej długości
...
Few-shot (tryb kilku przykładów) to technika, w której przed właściwym pytaniem podajesz modelowi 2–5 par pytanie-odpowiedź. Model uczy się wzorca z przykładów i aplikuje go do nowego przypadku.
...
Chain-of-Thought (łańcuch myśli, w skrócie CoT) to technika, w której prosisz model o rozpisanie kroków wnioskowania przed podaniem odpowiedzi końcowej. Znacząco poprawia jakość odpowiedzi przy zadaniach wymagających logiki, obliczeń lub wieloetapowego rozumowania.
...
| Few-shot | 2–5 | Niestandarowe formaty, niuansowane klasyfikacje | Średni |
...
Dobra struktura promptu to nie format dla samego formatu – to sposób na ograniczenie przestrzeni decyzyjnej modelu. Im precyzyjniej zdefiniujesz, czego chcesz, tym mniej modelu pozostaje do „domyślenia się".

Najważniejsza zasada: **mów modelowi, co ma robić, nie czego ma unikać.** Zamiast „nie pisz ogólnikowo" użyj „każda sekcja musi zawierać co najmniej jedną liczbę z roku i źródłem". Instrukcje pozytywne działają lepiej niż negacje – model nie ma dobrego mechanizmu do ignorowania zabronionych wzorców, ale świetnie radzi sobie z replikowaniem wzorców pozytywnych.
...
Tree of Thoughts (drzewo myśli, ToT) to technika, w której model generuje wiele alternatywnych ścieżek wnioskowania jednocześnie, ocenia je i wybiera najlepszą. Zamiast jednego łańcucha myśli, dostaje wiele rozgałęzionych opcji.
...
To podejście szczególnie dobrze sprawdza się przy ocenie strategii, gdzie istnieje kilka równoprawnych perspektyw i ryzyko przeoczenia ważnego czynnika jest wysokie. Badania na benchmarku Game of 24 pokazały, że ToT poprawia skuteczność modelu z 7% (dla standardowego CoT) do 74%.
...
<aside class="callout-fact">
  ...
  <div class="callout-body">
    ...
    <p>Stanford DSPy (Declarative Self-improving Python, 2023) pokazał, że automatyczna optymalizacja promptów przez algorytm BootstrapFewShot podnosi stabilność odpowiedzi w testach A/B o 82% w porównaniu do ręcznie pisanych promptów. <strong>Frameworki do automatycznej kompilacji promptów są skuteczniejsze niż ręczne majsterkowanie, gdy masz dostęp do zestawu testowego z co najmniej 20 przykładami.</strong></p>
  </div>
</aside>
...
Ton: ekspercki, konkretny, bez filler text.
...
**Zamiast „pogrupuj te frazy sensownie" – zdefiniuj kryteria grupowania i format wyjścia.** Wtedy możesz porównywać wyniki między sesjami i wychwycić niespójności.
...
Wprowadź burstiness: zróżnicuj radykalnie długość zdań (mix 5-słowowych punchline'ów 
i 25-słowowych zdań złożonych). Usuń wszystkie zdania zaczynające się od "Warto".
Zamień pasywne konstrukcje na aktywne. Zachowaj wszystkie liczby i nazwy własne.
```

Listę wzorców do sprawdzenia po wygenerowaniu treści przez LLM:

- **Filler phrases** – „warto podkreślić", „nie sposób nie wspomnieć", „jak wszyscy wiemy" → usuń
- **Równomierna długość zdań** – jeśli każdy akapit ma 3-4 zdania o zbliżonej długości → różnicuj
- **Pasywne konstrukcje** – „zostało wykazane" zamiast „badanie wykazało" → przepisz na aktywne
...
- **Model skwarantannowany** – przetwarza zewnętrzne dane i ekstrahuje z nich ustrukturyzowane informacje w formacie JSON, ale nie ma dostępu do żadnych narzędzi

Uprzywilejowany model dostaje tylko wynik ekstrakcji od skwarantannowanego – strukturę JSON, nie surowy tekst. Złośliwa instrukcja ukryta w zewnętrznym dokumencie jest nieaktywna, bo trafia wyłącznie do modelu bez uprawnień.
...
|---|---|---|---|
| GPT-4o / GPT-5 | Instrukcje złożone, formatowanie | Bezpośredni, zorientowany na zadanie | Definiuj osobowość i styl explicite |
...
3. Zmień jeden element promptu (np. tylko format wyjścia, tylko definicję roli, tylko instrukcję długości)
...
- **Odpowiedzi są za długie lub za krótkie** mimo braku instrukcji długości → dodaj explicit `"maksymalnie X zdań"`
- **Format zmienia się między sesjami** → dodaj example answer lub szczegółową specyfikację formatowania
