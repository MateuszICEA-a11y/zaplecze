...
type Recorder = {
  ...
  state: { busy: boolean; live: boolean; capture: { storageError: boolean } };
};
...
function createHarness(): Harness {
  ...
  const sandbox: Sandbox = {
    ...
    location: { hostname: 'chatgpt.com', pathname: '/', href: 'https://chatgpt.com/', origin: 'https://chatgpt.com' },
    navigator: {},
    document: {
      getElementById: () => null,
      querySelector: () => null,
...
`;
  ...
  return { api: sandbox.__fanoutRecorderTest as Recorder, sandbox, storage };
}
...
describe('fanout recorder SSE parser', () => {
  it('captures a query from a standard LF data frame', async () => {
    ...
    await tap(harness, streamFrom(frame({ conversation_id: conversationId, id: 'msg-lf', search_model_queries: { queries: ['one query'] } })));
    const batches = await waitFor(() => harness.api.readQueries(conversationId), (value) => value.length === 1);
  ...
  it('handles CRLF delimiters and JSON split across byte chunks', async () => {
    const harness = createHarness();
    const payload = JSON.stringify({ conversation_id: 'conv-crlf', id: 'msg-crlf', search_model_queries: { queries: ['split query'] } });
    const split = Math.floor(payload.length / 2);

    await tap(harness, streamFrom(`data: ${payload.slice(0, split)}`, `${payload.slice(split)}\r\n\r\n`));
    const batches = await waitFor(() => harness.api.readQueries('conv-crlf'), (value) => value.length === 1);
  ...
  it('accepts data: without a space', async () => {
    ...
    await tap(harness, streamFrom(`data:${JSON.stringify({ conversation_id: 'conv-nospace', id: 'msg-nospace', search_model_queries: { queries: ['no space'] } })}\n\n`));
    const batches = await waitFor(() => harness.api.readQueries('conv-nospace'), (value) => value.length === 1);
  ...
  it('handles CR and LF arriving in separate network chunks', async () => {
    const harness = createHarness();
    const payload = JSON.stringify(nestedMessage('msg-boundary', ['zażółć gęślą'], 'conv-boundary'));
    await tap(harness, streamFrom(`data: ${payload}\r`, '\n\r', '\n'));
    expect(harness.api.readQueries('conv-boundary')[0]).toMatchObject({
      id: 'msg-boundary', queries: ['zażółć gęślą'],
    });
  });
  ...
  it('joins multiline SSE data before parsing JSON', async () => {
    ...
    const batches = await waitFor(() => harness.api.readQueries('conv-multiline'), (value) => value.length === 1);
  ...
  it('walks array payloads and records nested messages', async () => {
    const harness = createHarness();
    const payload = [{ conversation_id: 'conv-array' }, nestedMessage('msg-array', ['array query'])];

    await tap(harness, streamFrom(frame(payload)));
    const batches = await waitFor(() => harness.api.readQueries('conv-array'), (value) => value.length === 1);
  ...
  it('buffers a query seen before the conversation_id in a new chat', async () => {
    ...
    await tap(
      harness,
      streamFrom(frame(nestedMessage('msg-buffered', ['buffered query'])), frame({ conversation_id: 'conv-buffered' })),
    );
    const batches = await waitFor(() => harness.api.readQueries('conv-buffered'), (value) => value.length === 1);
  ...
  it('uses the enclosing message id for metadata and updates one batch per message', async () => {
    ...
    expect(batches).toHaveLength(1);
    expect(batches[0]).toMatchObject({ id: 'msg-parent', queries: ['first query', 'updated query'] });
  });
  ...
  it('associates path updates with the message id captured earlier in the stream', async () => {
    ...
    expect(batches).toHaveLength(1);
    expect(batches[0]).toMatchObject({ id: 'msg-path', queries: ['full query', 'path query', 'path array query'] });
  });
  ...
  it('keeps same-query batches separate when message ids differ', async () => {
    ...
    const batches = await waitFor(() => harness.api.readQueries('conv-ids'), (value) => value.length === 2);

    expect(batches.map((batch) => batch.id)).toEqual(['msg-a', 'msg-b']);
    expect(batches.map((batch) => batch.queries)).toEqual([['identical query'], ['identical query']]);
  });
});

describe('fanout recorder fetch hook', () => {
  it.each([
    ['a URL object', () => ({ input: new URL('https://chatgpt.com/backend-api/f/conversation'), init: { method: 'POST' } })],
    ['a Request object', () => ({ input: new Request('https://chatgpt.com/backend-api/conversation', { method: 'POST' }), init: undefined })],
    ['a trailing slash and query string', () => ({ input: '/backend-api/f/conversation/?mode=test', init: { method: 'POST' } })],
  ])('matches POST conversation requests passed as %s', async (_label, makeInput) => {
    const harness = createHarness();
    const body = frame({ conversation_id: 'conv-hook', id: 'msg-hook', search_model_queries: { queries: ['hook query'] } });
    ...
    const batches = await waitFor(() => harness.api.readQueries('conv-hook'), (value) => value.length === 1);
  ...
  it('preserves a rejected fetch promise', async () => {
    ...
    await expect(harness.sandbox.fetch('/backend-api/f/conversation', { method: 'POST' })).rejects.toBe(error);
  });

  it('leaves a cross-origin conversation response untouched', async () => {
    const harness = createHarness();
    const original = new Response(frame({ conversation_id: 'conv-cross-origin', id: 'msg-cross-origin', search_model_queries: { queries: ['ignore me'] } }), {
      headers: { 'content-type': 'text/event-stream' },
    });
    harness.sandbox.fetch = vi.fn(() => Promise.resolve(original));
    harness.api.installStreamHook();

    const returned = await harness.sandbox.fetch('https://other.example/backend-api/conversation', { method: 'POST' });
    expect(returned).toBe(original);
  ...
  it('marks storage failures in capture diagnostics without throwing from fetch', async () => {
    ...
    await tap(harness, streamFrom(frame({ conversation_id: 'conv-storage', id: 'msg-storage', search_model_queries: { queries: ['cannot save'] } })));
  ...
  it('decides capture from live mode when the request starts', async () => {
    ...
    resolveResponse(new Response(frame({ conversation_id: 'conv-live-off', id: 'msg-live-off', search_model_queries: { queries: ['should stay out'] } }), { headers: { 'content-type': 'text/event-stream' } }));
  ...
  it('captures a request that started live even if live mode is toggled off while pending', async () => {
    ...
    const pending = harness.sandbox.fetch('/backend-api/f/conversation', { method: 'POST' });
    harness.api.state.live = false;
    resolveResponse(new Response(frame({ conversation_id: 'conv-live-on', id: 'msg-live-on', search_model_queries: { queries: ['capture me'] } }), { headers: { 'content-type': 'text/event-stream' } }));
    const returned = await pending;
    await returned.text();
    const batches = await waitFor(() => harness.api.readQueries('conv-live-on'), (value) => value.length === 1);
