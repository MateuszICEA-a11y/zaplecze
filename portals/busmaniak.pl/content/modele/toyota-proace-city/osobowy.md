---
title: "Toyota Proace City Verso – wersja osobowa, miejsca, ceny"
date: 2026-03-24
lastmod: 2026-03-24
description: "Toyota Proace City Verso to osobowa odmiana małego vana z gamy K9 – w wersjach Medium i Long, z 5 lub 7 miejscami zależnie od nadwozia i konfiguracji. Sprawdź, czym różni się od Berlingo."
draft: false
author: "marek-kowalczyk"
h1: "Toyota Proace City Verso – wersja osobowa, miejsca, ceny"
parent: "toyota-proace-city"
type: "page"
volume: 390
main_keyword: "Toyota Proace City osobowy"
image: "/images/toyota-proace-city-osobowy-hero.webp"
faq:
  - question: "Ile miejsc ma Toyota Proace City Verso?"
    answer: "Toyota Proace City Verso była oferowana w dwóch długościach nadwozia: Medium (4403 mm) i Long (4753 mm). W zależności od wersji i konfiguracji występuje jako 5-osobowa, a odmiana Long może oferować układ 5+2, czyli łącznie 7 miejsc."
  - question: "Czym różni się Toyota Proace City Verso od wersji dostawczej?"
    answer: "Verso to osobowa odmiana homologowana jako M1, z pełnym przeszkleniem i wykończoną kabiną pasażerską. Wersja dostawcza jest nastawiona na przewóz towaru, a różnice obejmują nie tylko zabudowę, ale też sposób użytkowania, rozliczenia i konfiguracje wyposażenia."
  - question: "Czy Toyota Proace City Verso to to samo co Citroën Berlingo osobowy?"
    answer: "To bliźniacza konstrukcja z rodziny K9, spokrewniona także z Peugeotem Rifterem i Oplem Combo Life. Samochody dzielą architekturę modelową i wiele rozwiązań technicznych, ale nie są identycznymi ofertami handlowymi – różnią się detalami, wyposażeniem, gwarancją i dostępnością wersji zależnie od rynku oraz rocznika."
lead: "Toyota Proace City Verso to osobowy van Toyoty z rodziny K9, blisko spokrewniony z Citroënem Berlingo, Peugeotem Rifterem i Oplem Combo Life. Występuje w dwóch długościach nadwozia – Medium i Long – a w dłuższej odmianie może oferować nawet 7 miejsc."
youtube: "2DDdOjFILLs"
youtube_title: "Toyota Proace CITY - gdzie tu Toyota?"
---
## Proace City Verso – czym jest i dla kogo

[Toyota Proace City](/modele/toyota-proace-city/) w odmianie Verso to osobowy van segmentu kompaktowego. Model zadebiutował w 2019 roku jako część rodziny bliźniaczych konstrukcji K9, a po zmianach ofertowych po 2023/2024 roku gama na poszczególnych rynkach zaczęła się wyraźniej różnić.

Verso oznacza wersję osobową z homologacją M1, pełnym przeszkleniem i kabiną przygotowaną do przewozu pasażerów. To propozycja dla rodzin, przewozu hotelowego, shuttle busów i firm, które potrzebują bardziej uniwersalnego auta niż klasyczny dostawczak.

Samochód był oferowany w dwóch długościach nadwozia:

| Wersja | Długość | Rozstaw osi | Miejsca | Układ siedzeń | Przesuwne drzwi |
|---|---:|---:|---:|---|---|
| Medium | 4403 mm | 2785 mm | 5 | 2+3 | Prawe, opcjonalnie lewe i prawe |
| Long | 4753 mm | 2975 mm | 5 lub 7 | 2+3 lub 2+3+2 | Prawe, opcjonalnie lewe i prawe |

Szerokość nadwozia bez lusterek wynosi ok. 1848 mm, a wysokość ok. 1812 mm. To ważne w codziennym użytkowaniu, bo auto zwykle mieści się jeszcze w typowych limitach parkingowych 1,9 m, choć przy niskich stropach warto zachować zapas.

## Silniki dostępne w Proace City Verso

### Warianty benzynowe i diesla

W poprzednich rocznikach Proace City Verso korzystał z tej samej rodziny napędów co pozostałe modele K9, ale dostępność silników zależała od rynku i momentu produkcji. Nie można więc traktować dawnych konfiguracji benzynowych i wysokoprężnych jako pewnej, aktualnej oferty handlowej na każdy rynek.

Najbezpieczniej mówić o gamie historycznie obejmującej małe silniki benzynowe 1.2, diesle 1.5 oraz odmianę elektryczną. Nazewnictwo Toyoty i rzeczywista dostępność konkretnych mocy różniły się od oznaczeń stosowanych bezpośrednio przez marki Stellantis.

W praktyce do jazdy rodzinnej i pod większym obciążeniem lepiej pasują mocniejsze warianty wysokoprężne albo elektryczna wersja użytkowana głównie w mieście. Przy 7 osobach i komplecie bagażu kluczowe stają się nie tylko osiągi, ale też elastyczność oraz poziom hałasu przy prędkościach autostradowych.

### Wersja elektryczna e-Proace City Verso

Osobowa odmiana elektryczna to e‑Proace City Verso. Korzysta z akumulatora o pojemności ok. 50 kWh, a deklarowany zasięg WLTP zależy od długości nadwozia i wersji – typowo do ok. 280–284 km WLTP, a nie jedna stała wartość dla całej gamy.

Ważniejsze od samej mocy jest tu ładowanie. W zależności od wersji spotyka się ładowanie AC 7,4 lub 11 kW, szybkie ładowanie DC do 100 kW oraz czas uzupełnienia energii od 0 do 80% na poziomie około 30 minut w optymalnych warunkach.

{{% info title="Proace City Verso a homologacja M1" icon="engineering" %}}
Homologacja M1 oznacza, że mówimy o samochodzie osobowym, ale praktyczne skutki zależą od sposobu użytkowania pojazdu. Kwestie tachografu, rozliczeń VAT czy obowiązków przewozowych wynikają przede wszystkim z DMC, rodzaju wykonywanego transportu i tego, czy auto pracuje zarobkowo, a nie z samego oznaczenia M1. W typowym prywatnym lub firmowym użytkowaniu osobowego vana do 3,5 t tachograf zwykle nie jest wymagany. Badania techniczne dla M1 odbywają się co do zasady po 3 latach od pierwszej rejestracji, potem po 2 latach, a następnie co roku.
{{% /info %}}

## Przestrzeń pasażerska i bagażnik

![Toyota Proace City Verso Long – wnętrze z trzecim rzędem siedzeń](/images/toyota-proace-city-osobowy-hero.webp)

Najważniejsza różnica praktyczna między Medium a Long dotyczy nie tylko liczby miejsc, ale też bagażnika. Dla bliźniaczych modeli K9 producenci podawali zwykle około 597 l VDA dla krótszej wersji 5-miejscowej oraz około 850 l VDA dla dłuższej wersji 5-miejscowej.

Przy 7 miejscach w odmianie Long przestrzeń za trzecim rzędem staje się wyraźnie mniejsza. To układ dobry na okazjonalne przewiezienie dodatkowych pasażerów, ale na rodzinny wyjazd w komplecie osób trzeba liczyć się z mocnym ograniczeniem miejsca na bagaże.

Trzeci rząd w tej klasie aut najlepiej sprawdza się dla dzieci albo na krótszych dystansach. Dorośli zmieszczą się tam awaryjnie, jednak nie będzie to tak pełnowymiarowa przestrzeń jak w większych vanach segmentu D.

Układ siedzeń zależy od rynku i konfiguracji. W części wersji drugi rząd może mieć formę dzielonej kanapy, w innych występują osobne miejsca, a możliwość składania lub wyjmowania poszczególnych foteli nie jest identyczna w każdej odmianie.

Duże przesuwne drzwi i wysoka pozycja dachu ułatwiają codzienne wsiadanie dzieciom oraz starszym pasażerom. Z drugiej strony trzeba pamiętać, że to wysoki van – przy autostradowych 140 km/h komfort akustyczny zwykle ustępuje klasycznym kombi i SUV-om.


{{% youtube %}}

## Ceny i wyposażenie

### Cennik Proace City Verso

Nie podajemy tu konkretnych kwot startowych, bo wcześniejsze wartości rzędu 100–115 tys. zł są dziś niewiarygodne jako uniwersalny punkt odniesienia dla nowego auta osobowego tej klasy. Ceny Proace City Verso zależą od rocznika, rynku, napędu, długości nadwozia i aktualnej polityki importera.

Jeśli interesuje Cię nowy egzemplarz, trzeba sprawdzić bieżący polski cennik Toyoty z dnia zakupu. Po zmianach ofertowych po 2023/2024 roku gama osobowych vanów była na wielu rynkach ograniczana, a część konfiguracji mogła zniknąć lub być dostępna wyłącznie na zamówienie.

### Linie wyposażenia

Nazwy wersji wyposażenia i ich zakres zmieniały się zależnie od rynku oraz rocznika, dlatego nie warto bez weryfikacji przyjmować dawnych nazw jako obowiązujących. W praktyce kluczowe jest to, czy dany egzemplarz ma zestaw systemów bezpieczeństwa i funkcji rodzinnych, a nie sama etykieta wersji.

Warto szukać takich elementów jak automatyczna klimatyzacja, kamera cofania, przesuwne drzwi po obu stronach, czujniki parkowania oraz systemy ADAS. W tej klasie szczególnie przydatne są AEB, utrzymanie pasa ruchu, rozpoznawanie znaków, tempomat adaptacyjny, monitoring martwego pola i kamera o szerokim kącie widzenia.

{{% expert name="Marek Kowalczyk" %}}
Jeśli szukasz Toyota Proace City osobowy dla rodziny, najpraktyczniejsza jest odmiana Long – zwłaszcza wtedy, gdy planujesz okazjonalnie wozić 7 osób. Trzeci rząd traktowałbym jednak jako rozwiązanie dodatkowe, bo po jego rozłożeniu bagażnik mocno się kurczy. Przed zakupem koniecznie sprawdź też konkretny układ drugiego rzędu siedzeń, bo to właśnie on najmocniej wpływa na codzienną wygodę.
{{% /expert %}}

Bezpośrednia konkurencja to [Citroën Berlingo osobowy](/modele/citroen-berlingo/), a także Peugeot Rifter i Opel Combo Life. To bliźniacze konstrukcje K9 o tych samych długościach 4403 i 4753 mm, ale mogą różnić się napędami, nazwami wersji wyposażenia, warunkami gwarancji, cenami katalogowymi i kosztami serwisu.

Toyota zwykle wygrywa marką, siecią dealerską i postrzeganą obsługą posprzedażową, natomiast konkurenci z rodziny Stellantis częściej mają szerszą rozpiętość konfiguracji zależnie od rynku. Dlatego przy zakupie warto porównywać nie sam model bazowy, lecz konkretną wersję nadwozia, liczbę miejsc i wyposażenie bezpieczeństwa.