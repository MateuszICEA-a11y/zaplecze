---
title: "Toyota Proace City – dane techniczne, wymiary, silniki"
date: 2026-03-24
lastmod: 2026-03-24
description: "Toyota Proace City – kompaktowy dostawczak na platformie PSA. Wymiary, silniki 1.2 i 1.5, wersja elektryczna, porównanie z Berlingo i Kangoo."
draft: false
author: "marek-kowalczyk"
h1: "Toyota Proace City – kompletny przewodnik"
parent: ""
type: "page"
volume: 6600
main_keyword: "Toyota Proace City"
image: "/images/toyota-proace-city-hero.jpg"
image_alt: "Toyota Proace City furgon"
lead: "Toyota Proace City to kompaktowy furgon zbudowany na platformie PSA EMP2 – bliźniak Peugeota Partner III i Citroëna Berlingo III. Wybierasz między wersjami Short i Long, trzema rodzajami napędu oraz gamą silników od 75 do 136 KM."
faq:
  - question: "Ile kosztuje Toyota Proace City?"
    answer: "Ceny startują od około 95 000 zł brutto za wersję Short z silnikiem 1.5 D-4D 75 KM. Elektryczny Proace City Electric kosztuje od około 175 000 zł brutto."
  - question: "Ile pali Toyota Proace City 1.5 diesel?"
    answer: "W cyklu mieszanym WLTP silnik 1.5 D-4D 100 KM zużywa około 5,5–6,2 l/100 km. W praktyce przy jeździe miejskiej spodziewaj się 6,5–7,5 l/100 km."
  - question: "Czy Proace City mieści europaletę?"
    answer: "Wersja Short mieści jedną europaletę (1200×800 mm) przy złożonych przednich siedzeniach pasażera. Wersja Long mieści dwie europalety – to kluczowa różnica przy wyborze długości nadwozia."
sources:
  - "Wikipedia – Toyota ProAce City"
  - "toyota.pl"
  - "auto-data.net."
---

## Historia i konstrukcja Toyota ProAce City

Toyota weszła na segment kompaktowych dostawczaków w 2020 roku, przejmując gotową architekturę Stellantis (dawniej PSA). [Toyota Proace City](https://pl.wikipedia.org/wiki/Toyota_Proace_City) produkowany jest w fabryce w Vigo w Hiszpanii – tej samej, która składa Partnera i Berlingo. To decyzja biznesowa, nie kompromis techniczny: platforma EMP2 to dojrzała konstrukcja, sprawdzona w milionach egzemplarzy.

Czym Proace City wyróżnia się spośród bliźniaków:

- sieć serwisowa Toyoty i gwarancja 5 lat lub 100 000 km (zależnie od pakietu)
- system Toyota Safety Sense w standardzie – automatyczne hamowanie, asystent pasa ruchu, rozpoznawanie znaków
- interfejs multimediów Toyota Touch 2 z Apple CarPlay i Android Auto
- opcjonalny napęd elektryczny (Proace City Electric) przy zachowaniu tych samych wymiarów zewnętrznych

Proace City dostępny jest w dwóch wersjach nadwozia – furgon oraz osobowy (Proace City Verso). Ten przewodnik dotyczy wersji dostawczej. Wariant [osobowy opisujemy osobno](/modele/toyota-proace-city/osobowy/).

## Wymiary Toyoty ProAce City


Kluczowy wybór przy zakupie Proace City to długość nadwozia. Różnica 350 mm między Short a Long przekłada się na dodatkową europaletę i 0,6 m³ przestrzeni ładunkowej.

| Parametr | Short | Long |
|---|---|---|
| Długość całkowita | 4403 mm | 4753 mm |
| Szerokość z lusterkami | 2107 mm | 2107 mm |
| Szerokość bez lusterek | 1848 mm | 1848 mm |
| Wysokość | 1803–1835 mm | 1820–1850 mm |
| Pojemność ładunkowa | 3,3 m³ | 3,9 m³ |
| Ładowność | do 1000 kg | do 1000 kg |
| Europalety (1200×800) | 1 szt. | 2 szt. |
| DMC | 1800–2200 kg | 1800–2200 kg |

Szerokość przestrzeni ładunkowej między nadkolami wynosi 1228 mm – wystarczy na ładunki w standardowych pojemnikach L. Podłoga jest płaska, próg załadunku niski, a opcjonalne drzwi boczne przesuwne (prawe lub obustronne) ułatwiają załadunek w ciasnych miejscach.

{{% info title="Proace City Long – kiedy warto dopłacić?" icon="engineering" %}}
Wersja Long kosztuje zwykle 3000–5000 zł więcej niż Short. Jeśli regularnie przewozisz europalety albo długie elementy (rury, profile, deski), zwrot z tej inwestycji następuje po kilku tygodniach pracy. Jeśli dominuje przewóz paczek i drobnego towaru – Short w zupełności wystarczy, a łatwiej nim manewrować w mieście.
{{% /info %}}

Szczegółową specyfikację wersji Long – w tym przestrzeń między łukami kół i opcje zabudowy – znajdziesz na stronie [Toyota Proace City Long](/modele/toyota-proace-city/long/).

## Silniki Toyoty ProAce City

### Benzyna – 1.2 PureTech

Trzycylindrowy turbodoładowany agregat benzynowy o pojemności 1199 cm³. Dostępny w dwóch wariantach:

- 110 KM / 205 Nm – wystarczający do zadań miejskich i regionalnych przy niskim obciążeniu
- 130 KM / 230 Nm – lepszy wybór przy częstej jeździe załadowanym pojazdem lub po trasach

PureTech to silnik skonstruowany przez PSA, stosowany w całej rodzinie bliźniaków. Skrzynia manualna 6-biegowa lub 8-biegowa automatyczna. Spalanie w cyklu WLTP od 6,0 l/100 km przy 110 KM.

### Diesel – 1.5 D-4D

Czterocylindrowy olej napędowy o pojemności 1499 cm³, dostępny w trzech stanach mocy:

- 75 KM / 230 Nm – wersja podstawowa, najniższy koszt zakupu i ubezpieczenia, wystarczająca do stałych tras miejskich
- 100 KM / 250 Nm – kompromis między ekonomiką a dynamiką, najpopularniejszy wariant w Polsce
- 130 KM / 300 Nm – przeznaczony do częstej jazdy z pełnym ładunkiem lub holowania przyczepy

Silnik 1.5 D-4D 100 KM w cyklu mieszanym WLTP spala 5,5–6,2 l/100 km. Norma emisji: Euro 6d. Dostępna skrzynia 6-biegowa manualna lub 8-biegowa automatyczna (EAT8).

{{% expert name="Marek Kowalczyk" %}}
W segmencie kompaktowych dostawczaków diesel wciąż dominuje – zwłaszcza przy przebiegu powyżej 40 000 km rocznie. Proace City 1.5 D-4D 100 KM to aktualnie najchętniej wybierany wariant w Polsce: dobry balans mocy, rozsądne spalanie i niski koszt serwisu dzięki sieci Toyoty. Jeśli twój przebieg jest niższy i poruszasz się głównie w mieście, rozważ elektryka – zwłaszcza gdy masz dostęp do ładowania w miejscu pracy.
{{% /expert %}}


![Toyota Proace City furgon](/images/toyota-proace-city-hero.jpg)

## ProAce City Electric

Toyota Proace City Electric korzysta z tego samego silnika elektrycznego co Peugeot e-Partner i Citroën ë-Berlingo. Parametry:

- pojemność baterii: 50 kWh (netto)
- moc: 136 KM / 260 Nm
- zasięg WLTP: do 280 km (Short), do 265 km (Long)
- ładowanie AC: do 11 kW (Mode 3), pełne ładowanie w ~5,5 h
- ładowanie DC: do 100 kW, naładowanie do 80% w ~30 min
- DMC: 2195–2285 kg

Proace City Electric zachowuje identyczne wymiary zewnętrzne i tę samą pojemność ładunkową co wersja spalinowa. Bateria umieszczona jest pod podłogą, co nie ogranicza przestrzeni ładunkowej ani ładowności w stosunku do wersji Short spalinowej.

Elektryczny Proace City kwalifikuje się do polskich dopłat w programach wsparcia flot zeroemisyjnych. Całkowity koszt posiadania (TCO) w 5-letnim cyklu eksploatacyjnym przy przebiegu 30 000 km/rok jest zbliżony do wariantu 1.5 D-4D 100 KM – różnica zależy głównie od kosztu energii elektrycznej vs oleju napędowego.

## Porównanie ProAce City z konkurencją

Proace City konkuruje bezpośrednio z bliźniakami na platformie EMP2 oraz z Renault Kangoo IV.

| Cecha | Toyota Proace City | Peugeot Partner III | Citroën Berlingo III | Opel Combo E | Renault Kangoo IV |
|---|---|---|---|---|---|
| Platforma | PSA EMP2 | PSA EMP2 | PSA EMP2 | PSA EMP2 | CMF-CD |
| Dług. Short | 4403 mm | 4403 mm | 4403 mm | 4403 mm | 4490 mm |
| Poj. ład. Short | 3,3 m³ | 3,3 m³ | 3,3 m³ | 3,3 m³ | 3,5 m³ |
| Elektryk | 50 kWh / 280 km | 50 kWh / 280 km | 50 kWh / 275 km | 50 kWh / 280 km | 45 kWh / 285 km |
| Gwarancja | 5 lat / 100 000 km | 3 lata / 60 000 km | 3 lata / 60 000 km | 3 lata / 100 000 km | 3 lata / 100 000 km |
| Sieć serwisowa | Toyota (duża) | Peugeot (duża) | Citroën (duża) | Opel (średnia) | Renault (duża) |

Różnice między bliźniakami na platformie EMP2 są minimalne pod względem technicznym. Proace City wyróżnia się gwarancją Toyoty i systemem bezpieczeństwa Toyota Safety Sense w standardzie. Renault Kangoo IV to odrębna platforma – nieco większa przestrzeń ładunkowa w wersji Short, ale krótsza historia niezawodności nowej generacji.

Jeśli interesuje cię większy dostawczak Toyoty – sprawdź [Toyota Proace](/modele/toyota-proace/), który oferuje przestrzeń ładunkową od 5,1 do 6,1 m³.

## Ceny Toyoty ProAce City

Zakup Toyota Proace City to wydatek, który zależy od generacji, wersji silnikowej i stanu technicznego – rozstrzał cenowy na rynku wtórnym bywa spory.

| Rocznik / Wersja | Cena orientacyjna |
|-------------------|-------------------|
| ProAce City (2020–2022) | 55 000–80 000 zł |
| ProAce City (2023–2025) | 75 000–110 000 zł |
| Nowy ProAce City Van Short | od ok. 100 000 zł netto |
| Nowy ProAce City Van Long | od ok. 110 000 zł netto |
| ProAce City Electric | od ok. 175 000 zł netto |
